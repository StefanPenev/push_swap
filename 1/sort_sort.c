...
void move_smallest_to_top(a_list **a) {
    int min = find_min(*a);
...
void bucket_sort(a_list **a, a_list **b) {
    ...
    // Split into 5 buckets (or more if needed)
    int bucket_size = size / 5;
    if (bucket_size == 0) bucket_size = 1; // For small lists

    for (int i = 0; i < 5; i++) {
        int moves = 0;

        // Move elements to stack b if they belong to the current bucket
        while (moves < bucket_size && stack_size(*a) > 0) {
            int current = (*a)->content;
            if (current >= find_min(*a) && current <= (find_min(*a) + bucket_size)) {
                pb(a, b);
                moves++;
            } else {
                ra(a);
        ...
        // Sort elements in stack b and move back to stack a
        while (stack_size(*b) > 0) {
            move_smallest_to_top(b);
            pa(a, b);
        }
    }

    // Ensure the remaining elements are sorted
    while (!is_sorted(*a)) {
